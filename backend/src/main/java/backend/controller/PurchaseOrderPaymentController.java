 ...
 *
 * Updating and deleting of purchase orders can influence the balance of the corresponding payment account. This manager
 * creates postings and updates the balance of the payment account on relevant purchase order status changes.
 *
 * @author Michael
 */
public class PurchaseOrderPaymentController {
     ...
     */
    public static String getReferenceNumber(final PurchaseOrder purchaseOrder) {
     ...
     */
    public void updatePurchaseOrderPayment(final PurchaseOrder purchaseOrder, final PurchaseOrder databasePurchaseOrder)
            throws Exception {
        ...
        if (!databasePurchaseOrder.isStatusActive(PurchaseOrderStatus.INVOICE_SETTLED)
                && purchaseOrder.isStatusActive(PurchaseOrderStatus.INVOICE_SETTLED)) {
     ...
     */
    private void reduceAccountBalance(final PurchaseOrder purchaseOrder) throws ObjectUnchangedException, Exception {
        AccountDao accountDAO = DAOManager.getInstance().getAccountDAO();
        Account paymentAccount = purchaseOrder.getPaymentAccount();

        paymentAccount
                .setBalance(purchaseOrder.getPaymentAccount().getBalance().subtract(purchaseOrder.getPriceTotal()));
     ...
     * @throws ObjectUnchangedException The account has not been changed.
     */
    public void increaseAccountBalance(final PurchaseOrder purchaseOrder) throws ObjectUnchangedException, Exception {
        AccountDao accountDAO = DAOManager.getInstance().getAccountDAO();
        Account paymentAccount = purchaseOrder.getPaymentAccount();
     ...
     * @return The posting.
     */
    private Posting getPosting(final PurchaseOrder purchaseOrder, final PostingType type) {
        Posting posting;
