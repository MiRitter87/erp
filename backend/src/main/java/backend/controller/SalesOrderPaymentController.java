 ...
 *
 * Updating and deleting of sales orders can influence the balance of the corresponding payment account. This manager
 * creates postings and updates the balance of the payment account on relevant sales order status changes.
 *
 * @author Michael
 */
public class SalesOrderPaymentController {
     ...
     */
    public static String getReferenceNumber(final SalesOrder salesOrder) {
     ...
     */
    public void updateSalesOrderPayment(final SalesOrder salesOrder, final SalesOrder databaseSalesOrder)
            throws Exception {
        // If the sales order status changes to "Finished", a posting of payment receipt is created and the total price
        // is added to the payment account.
        if (databaseSalesOrder.getStatus() != SalesOrderStatus.FINISHED
                && salesOrder.getStatus() == SalesOrderStatus.FINISHED) {
            this.addPaymentToAccount(salesOrder);
            return;
        }

        // If the sales order status "Finished" is reverted, a posting of payment disbursal is created and the total
        // price is subtracted from the payment account.
        if (databaseSalesOrder.getStatus() == SalesOrderStatus.FINISHED
                && salesOrder.getStatus() != SalesOrderStatus.FINISHED) {
     ...
     */
    private void addPaymentToAccount(final SalesOrder salesOrder) throws ObjectUnchangedException, Exception {
     ...
     * @throws ObjectUnchangedException The account has not been changed.
     */
    public void removePaymentFromAccount(final SalesOrder salesOrder) throws ObjectUnchangedException, Exception {
        AccountDao accountDAO = DAOManager.getInstance().getAccountDAO();
        Account paymentAccount = salesOrder.getPaymentAccount();
     ...
     * @return The posting.
     */
    private Posting getPosting(final SalesOrder salesOrder, final PostingType type) {
        Posting posting;
